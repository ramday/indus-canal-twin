...
Includes digital low-pass filtering and automatic accumulator reset upon alarm trigger for sequential multi-event detection.
"""
...
class CUSUMDetector:
    ...
    Resets the test statistic S+ to 0 upon crossing threshold H to enable sequential multi-event detection.
    """
    def __init__(
        self,
        ...
        filter_alpha: float = 0.15  # Low-pass filter smoothing factor (0 < alpha <= 1)
    ):
        self.net = network
    ...
    def smooth_signal(self, signal: np.ndarray) -> np.ndarray:
        """Applies a first-order exponential low-pass filter to sensor time series."""
    ...
    def compute_residuals(
        ...
        """
        Filters raw noisy telemetry and computes the physical mass conservation residual for Reach 2.
        """
        ...
        for k in range(1, n_steps):
            ...
            # 3. Filtered storage rate: dV/dt = Area * (dh/dt)
    ...
    def detect_anomalies(self, residuals: np.ndarray, reset_on_alarm: bool = True) -> Dict[str, Any]:
        """
        Executes CUSUM recursion S_k^+ = max(0, S_{k-1}^+ + (r_k - mu_0 - K)).
        When reset_on_alarm is True, resets S_k^+ to 0 upon crossing H,
        enabling independent sequential detection of subsequent theft events.
        """
        ...
        for k in range(1, n_steps):
            ...
            if s_pos[k] >= self.threshold_h:
                ...
                if reset_on_alarm:
                    # Reset accumulator so subsequent events start from baseline 0
                    s_pos[k] = 0.0
